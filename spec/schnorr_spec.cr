...
# Official BIP-340 test vectors (bip-0340/test-vectors.csv).
# {index, public key, message, signature, expected, comment}
# Vectors 15-18 are omitted: their messages are not 32 bytes, which the
# Nostr-facing API rejects up front.
BIP340_VECTORS = [
  ...
  {3, "25D1DFF95105F5253C4022F628A996AD3A0D95FBF21D468A1B33F8C160D8F517",
   ...
   true, "valid, msg must not be reduced modulo p or n"},
...
# A real Nostr event generated and signed with nak (secret key 0x03).
NOSTR_EVENT_ID     = "f0060a8d9405f319bc4d1302fb2ddeb63a082234287fa6e95cac92e5890b269b"
NOSTR_EVENT_PUBKEY = "f9308a019258c31049344f85f89d5229b531c845836f99b08601f113bce036f9"
NOSTR_EVENT_SIG    = "81c757181d3228911d241e67727748b8556d5d17d0b2863dfc0e8c8568140b28307a5e19abb4e475aa886bf204e0e6d9a2a06f92a458af65d9ca387d964bfac8"

describe Schnorr do
  describe ".valid_signature?" do
    BIP340_VECTORS.each do |index, pub, msg, sig, expected, comment|
      it "BIP-340 vector #{index} (#{comment})" do
        Schnorr.valid_signature?(msg.downcase, pub.downcase, sig.downcase).should eq(expected)
      end
    end

    it "accepts a real signed Nostr event" do
      Schnorr.valid_signature?(NOSTR_EVENT_ID, NOSTR_EVENT_PUBKEY, NOSTR_EVENT_SIG).should be_true
    end

    it "rejects the same event with one signature byte flipped" do
      tampered_r = NOSTR_EVENT_SIG.sub(0, "9")
      tampered_s = NOSTR_EVENT_SIG.sub(126, "0")
      Schnorr.valid_signature?(NOSTR_EVENT_ID, NOSTR_EVENT_PUBKEY, tampered_r).should be_false
      Schnorr.valid_signature?(NOSTR_EVENT_ID, NOSTR_EVENT_PUBKEY, tampered_s).should be_false
    end

    it "rejects a signature over a different message" do
      other = "0" * 63 + "1"
      Schnorr.valid_signature?(other, NOSTR_EVENT_PUBKEY, NOSTR_EVENT_SIG).should be_false
    end

    it "rejects malformed hex input" do
      Schnorr.valid_signature?("zz" * 32, NOSTR_EVENT_PUBKEY, NOSTR_EVENT_SIG).should be_false
      Schnorr.valid_signature?(NOSTR_EVENT_ID, NOSTR_EVENT_PUBKEY, "zz" * 64).should be_false
    end

    it "rejects inputs with the wrong length" do
      Schnorr.valid_signature?(NOSTR_EVENT_ID[0...62], NOSTR_EVENT_PUBKEY, NOSTR_EVENT_SIG).should be_false
      Schnorr.valid_signature?(NOSTR_EVENT_ID, NOSTR_EVENT_PUBKEY, NOSTR_EVENT_SIG[0...126]).should be_false
    end
  end
end
